...
import { CURATED_VISUALS } from "./seed";
...
const mapRanges: Record<string, [string, number, number][]> = {
  ...
  temple: [["1Ki", 5, 8]], jerusalem: [["Mat", 1, 28], ["Mrk", 1, 16], ["Luk", 1, 24], ["Jhn", 1, 21]], paul: [["Act", 1, 28]],
};
...
const maps: VisualAsset[] = STUDY_MAPS.map(map => {
  ...
  return {
    ...
    medium: reconstruction ? "Diagram or model photograph" : "Study map", description: map.caption,
    ...
    source: { name: "Wikimedia Commons", url: map.sourceUrl ?? "", license: map.license ?? "Not recorded", licenseUrl: legacyLicenseUrl(map.license ?? ""), credit: map.artist ?? "Attribution not recorded" },
...
const paintings: VisualAsset[] = BIBLE_PLATES.map(plate => ({
  ...
  source: { name: plate.source === "brooklyn" ? "Brooklyn Museum / Wikimedia Commons" : "Wikimedia Commons", url: plate.sourceUrl ?? "", license: plate.license === "pd" ? "Public domain (legacy label)" : plate.license ?? "Not recorded", licenseUrl: legacyLicenseUrl(plate.license ?? ""), credit: plate.artist ?? "Attribution not recorded" },
  review: "legacy", thumbnailUrl: biblePlateAssetUrl(plate), detailUrl: biblePlateAssetUrl(plate), alt: plate.alt,
}));

export const VISUAL_CATALOGUE: VisualAsset[] = deduplicateVisuals([...CURATED_VISUALS, ...maps, ...paintings]);
