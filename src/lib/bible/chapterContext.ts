...
export function platesForChapter(bookAbbr: string, chapter: number): BiblePlate[] {
  return BIBLE_PLATES.filter((p) => p.bookAbbr === bookAbbr && p.chapter === chapter)
    .sort((a, b) => a.beforeVerse - b.beforeVerse || (a.priority ?? 10) - (b.priority ?? 10));
}
/** The page-turning stream uses the visual library, not only the old plate collection. */
export function inlinePlatesForChapter(bookAbbr: string, chapter: number): BiblePlate[] {
  return selectReaderVisuals(bookAbbr, chapter, platesForChapter(bookAbbr, chapter));
}
export function platesBeforeVerse(bookAbbr: string, chapter: number, verse: number): BiblePlate[] {
...
export function hasChapterMedia(bookAbbr: string, chapter: number): boolean {
  return inlinePlatesForChapter(bookAbbr, chapter).length > 0 || mapsForChapter(bookAbbr, chapter).length > 0;
}
...
const TIMELINE_BY_BOOK: Record<string, ChapterTimelineEvent[]> = {
  Gen: [{ id: "gen-early-bronze", label: "Patriarchal era", approxYear: "2000–1800 BC", empire: "Egypt (Middle Kingdom)" }],
  Exo: [
  ...
  Isa: [{ id: "isa-assyria", label: "Assyrian threat", approxYear: "740–701 BC", empire: "Assyria" }],
  Jer: [{ id: "jer-babylon", label: "Babylonian siege", approxYear: "586 BC", empire: "Babylon" }],
  Dan: [
  ...
  Mat: [{ id: "nt-rome", label: "Roman Judea", approxYear: "4 BC – AD 30", empire: "Rome" }],
  Act: [{ id: "act-rome", label: "Early church under Rome", approxYear: "AD 30–62", empire: "Rome" }],
  Rev: [{ id: "rev-domitian", label: "Imperial persecution era", approxYear: "AD 90s", empire: "Rome" }],
};
...
export function mapsForChapter(bookAbbr: string, chapter: number): StudyMapEntry[] {
  const ids = new Set(CHAPTER_MAP_IDS.filter((r) => r.bookAbbr === bookAbbr && chapter >= r.chapterMin && chapter <= r.chapterMax).map((r) => r.mapId));
...
export function chapterContext(bookAbbr: string, chapter: number): ChapterContextBundle {
  return { bookAbbr, chapter, plates: platesForChapter(bookAbbr, chapter), mapIds: mapsForChapter(bookAbbr, chapter).map((m) => m.id), timeline: timelineForChapter(bookAbbr, chapter), relatedPassages: relatedPassagesForChapter(bookAbbr, chapter) };
}
