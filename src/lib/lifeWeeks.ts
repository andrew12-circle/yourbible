...
/** Poster subtitle, e.g. "April 10th, 2023" (UTC calendar date). */
export function formatBirthDatePoster(iso: string): string {
  const ms = parseBirthDate(iso);
  if (ms === null) return iso;
  const d = new Date(ms);
  const month = POSTER_MONTHS[d.getUTCMonth()] ?? "January";
  const day = d.getUTCDate();
  return `${month} ${ordinalDay(day)}, ${d.getUTCFullYear()}`;
}
