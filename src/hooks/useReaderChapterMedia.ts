...
import { chapterContext, inlinePlatesForChapter } from "@/lib/bible/chapterContext";
...
export function useReaderChapterMedia(bookAbbr: string, chapter: number) {
  ...
  return { chapterContextOpen, setChapterContextOpen, chapterCtx, showChapterContext: true, hasInlinePlates: inlineChapterPlates.length > 0, inlineChapterPlates };
}
