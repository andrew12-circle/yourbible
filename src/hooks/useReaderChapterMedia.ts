...
export function useReaderChapterMedia(bookAbbr: string, chapter: number) {
  ...
  return { chapterContextOpen, setChapterContextOpen, chapterCtx, showChapterContext: hasChapterMedia(bookAbbr, chapter) || hasVisualSeedForChapter(bookAbbr, chapter), hasInlinePlates: inlineChapterPlates.length > 0, inlineChapterPlates };
}
