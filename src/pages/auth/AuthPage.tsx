...
import { BookOpen, Brain, Loader2, NotebookPen } from "lucide-react";
...
const HIGHLIGHTS = [
  { icon: BookOpen, label: "Read", detail: "Beautiful Bible with highlights, notes, and audio" },
  { icon: NotebookPen, label: "Journal", detail: "Daily prompts and a private place to process life" },
  { icon: Brain, label: "Map beliefs", detail: "Framework + AI grounded in your own journey" },
] as const;
...
export default function AuthPage() {
  const { user, profile, signIn, signUp, requestPasswordReset, loading } = useAuth();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (mode === "signup" && !acceptedTerms) {
      toast({
        ...
        description: "Review the Privacy Policy and Terms before creating an account.",
      });
      return;
    }
    setBusy(true);
    ...
    <div className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden px-6 pb-[max(1.5rem,var(--safe-area-inset-bottom))] pt-[max(1.5rem,var(--safe-area-inset-top))] app-mesh">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-[15px] mb-5">{APP_DESCRIPTION}</p>
          <ul className="text-left space-y-2 max-w-xs mx-auto">
            {HIGHLIGHTS.map(({ icon: Icon, label, detail }) => (
              <li key={label} className="flex gap-2.5 text-xs text-muted-foreground">
                <Icon className="w-4 h-4 shrink-0 text-primary mt-0.5" strokeWidth={1.75} />
        ...
        {mode === "forgot" ? (
          <form onSubmit={submitForgot} className="space-y-4 bg-white/85 backdrop-blur-xl p-7 rounded-3xl border border-white/70 shadow-[0_30px_60px_-20px_rgba(15,23,42,0.18)]">
            ...
            <button type="button" onClick={() => setMode("signin")} className="w-full text-xs text-muted-foreground hover:text-primary transition-colors pt-2">
              Back to sign in
            </button>
          </form>
        ) : (
