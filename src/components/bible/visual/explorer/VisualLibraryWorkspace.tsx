...
function Workspace({ context, book, chapter, translation: abbreviation, ownerId, onNavigate, assets = VISUAL_CATALOGUE }: Props) {
  ...
  const placeChapter = section === "places" && placeScope === "passage" ? chapter : section === "places" ? undefined : activeChapter;
  ...
  const places = useMemo(() => explorerPlaces(atlas.data?.places ?? [], deferredQuery, placeBook, placeChapter, translation).filter(place => section !== "saved" || saved.ids.includes(`place:${place.id}`)), [atlas.data, deferredQuery, placeBook, placeChapter, translation, section, saved.ids]);
  ...
  const scopeLabel = section === "places"
    ? placeScope === "passage" && book ? `${contextName} ${chapter}` : libraryBook ? BOOKS.find(item => item.abbr === libraryBook)?.name : "All Bible places"
    : chapterOnly ? `${contextName} ${chapter}` : libraryBook ? BOOKS.find(item => item.abbr === libraryBook)?.name : "Entire library";
  ...
  return <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden md:flex-row" data-testid="visual-library-workspace">
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      <div className="shrink-0 space-y-3 border-b p-3 sm:p-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><h2 className="font-serif text-xl">{currentSection.label}</h2><p className="text-xs text-muted-foreground">{scopeLabel}</p></div><div className="flex gap-2">{book && section === "places" ? <select aria-label="Place scope" className={`${input} max-w-48`} value={placeScope} onChange={event => { setPlaceScope(event.target.value as "all" | "passage"); setLibraryBook(""); setLimit(24); }}><option value="all">All Bible places</option><option value="passage">This passage</option></select> : book ? <select aria-label="Library scope" className={`${input} max-w-48`} value={chapterOnly ? "chapter" : "all"} onChange={event => setChapterOnly(event.target.value === "chapter")}><option value="chapter">This chapter</option><option value="all">Entire library</option></select> : null}<button type="button" className={explorerButton} aria-expanded={refine} aria-controls={`${id}-filters`} onClick={() => setRefine(value => !value)} title="Refine the collection"><SlidersHorizontal className="h-4 w-4" /><span className="hidden sm:inline">Refine</span></button></div></div>
      ...
      <div ref={scroller} tabIndex={-1} className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-6" data-testid="visual-workspace-scroll">
        {selected ? <VisualArtworkView key={selected.id} asset={selected} index={selectedIndex} total={visuals.length} onStep={step} onBack={back} saved={saved.ids.includes(`visual:${selected.id}`)} onSave={() => saved.toggle(`visual:${selected.id}`)} onNavigate={onNavigate} places={selectedPlaces} /> : <>
          ...
          {section === "collections" ? <div className={galleryGrid}>{VISUAL_COLLECTIONS.map(item => { const records = filterCollection(scopeVisuals, item.id); const cover = records[0]; return <button type="button" key={item.id} className="group overflow-hidden rounded-2xl border bg-card text-left hover:shadow-md disabled:opacity-50" disabled={!records.length} onClick={() => { setCollection(item.id); setSection(item.id === "masterworks" ? "art" : item.id === "maps" ? "maps" : item.id === "artifacts" ? "objects" : item.id === "places" ? "places" : "discover"); }}><div className="h-44 bg-muted/30">{cover ? <VisualPreview asset={cover} /> : <div className="flex h-full items-center justify-center text-xs text-muted-foreground">No matching visuals in this scope</div>}</div><div className="space-y-2 p-5"><h3 className="font-serif text-xl">{item.title}</h3><p className="text-sm text-muted-foreground">{item.description}</p><p className="text-xs">{records.length} visuals</p></div></button>; })}</div> : <>
            {featured && <FeaturedVisual asset={featured} onSelect={select} />}
            {visibleVisuals.length > 0 && <><div className="mb-4 flex items-center justify-between gap-2"><h3 className="text-sm font-semibold">{section === "places" ? "Site photography" : featured ? "More connections" : "Browse the collection"}</h3>{section === "discover" && collection === "all" && visuals.length > 8 && <button type="button" className="min-h-11 text-sm underline" onClick={() => { setSection("art"); setCollection("all"); }}>Open art gallery</button>}</div><VisualGallery assets={visibleVisuals} saved={saved.ids} onSave={saved.toggle} onSelect={select} /></>}
            {(section !== "discover" || collection !== "all") && visuals.length > limit && <button type="button" className={`${explorerButton} mt-5 w-full`} onClick={() => setLimit(value => value + 24)}>Show more artwork ({visuals.length - limit} remaining)</button>}
