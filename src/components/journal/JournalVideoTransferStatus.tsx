    ...
    {currentRows.map((row) => {
      ...
      return <div key={row.id} className="rounded-xl border border-border/60 bg-muted/20 p-3" data-video-transfer={row.id}>
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          ...
          {paused ? <Button type="button" variant="ghost" className="min-h-11 gap-1.5 px-2 text-xs" onClick={() => retry(row)}><RotateCw className="h-3.5 w-3.5" />Retry now</Button> : null}
        </div>
      </div>;
    })}
