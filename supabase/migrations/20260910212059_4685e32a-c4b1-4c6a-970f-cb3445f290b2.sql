...
CREATE UNIQUE INDEX IF NOT EXISTS habit_notes_global_month ON public.habit_notes (user_id, year_month) WHERE habit_id IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS habit_notes_habit_month ON public.habit_notes (user_id, habit_id, year_month) WHERE habit_id IS NOT NULL;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.habit_bills TO authenticated;
GRANT ALL ON public.habit_bills TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.habit_badges TO authenticated;
GRANT ALL ON public.habit_badges TO service_role;

CREATE INDEX IF NOT EXISTS idx_habits_user_active ON public.habits (user_id, sort_order) WHERE archived_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_habit_completions_user_month ON public.habit_completions (user_id, completion_date DESC);
CREATE INDEX IF NOT EXISTS idx_habit_completions_habit ON public.habit_completions (habit_id, completion_date DESC);
CREATE INDEX IF NOT EXISTS idx_habit_goals_user_month ON public.habit_goals (user_id, year_month, goal_type);
CREATE INDEX IF NOT EXISTS idx_habit_notes_user_month ON public.habit_notes (user_id, year_month);
CREATE INDEX IF NOT EXISTS idx_habit_bills_user_month ON public.habit_bills (user_id, year_month);
CREATE INDEX IF NOT EXISTS idx_habit_badges_user ON public.habit_badges (user_id, unlocked_at DESC);
...
CREATE TRIGGER trg_habit_goals_updated BEFORE UPDATE ON public.habit_goals FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
DROP TRIGGER IF EXISTS trg_habit_notes_updated ON public.habit_notes;
CREATE TRIGGER trg_habit_notes_updated BEFORE UPDATE ON public.habit_notes FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
DROP TRIGGER IF EXISTS trg_habit_bills_updated ON public.habit_bills;
CREATE TRIGGER trg_habit_bills_updated BEFORE UPDATE ON public.habit_bills FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE POLICY "habit_completions_insert_own" ON public.habit_completions FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid() AND EXISTS (SELECT 1 FROM public.habits h WHERE h.id = habit_id AND h.user_id = auth.uid()));
...
CREATE POLICY "habit_goals_select_own" ON public.habit_goals FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_goals_insert_own" ON public.habit_goals;
CREATE POLICY "habit_goals_insert_own" ON public.habit_goals FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_goals_update_own" ON public.habit_goals;
CREATE POLICY "habit_goals_update_own" ON public.habit_goals FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_goals_delete_own" ON public.habit_goals;
CREATE POLICY "habit_goals_delete_own" ON public.habit_goals FOR DELETE TO authenticated USING (user_id = auth.uid());

DROP POLICY IF EXISTS "habit_notes_select_own" ON public.habit_notes;
CREATE POLICY "habit_notes_select_own" ON public.habit_notes FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_notes_insert_own" ON public.habit_notes;
CREATE POLICY "habit_notes_insert_own" ON public.habit_notes FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_notes_update_own" ON public.habit_notes;
CREATE POLICY "habit_notes_update_own" ON public.habit_notes FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_notes_delete_own" ON public.habit_notes;
CREATE POLICY "habit_notes_delete_own" ON public.habit_notes FOR DELETE TO authenticated USING (user_id = auth.uid());

DROP POLICY IF EXISTS "habit_bills_select_own" ON public.habit_bills;
CREATE POLICY "habit_bills_select_own" ON public.habit_bills FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_bills_insert_own" ON public.habit_bills;
CREATE POLICY "habit_bills_insert_own" ON public.habit_bills FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_bills_update_own" ON public.habit_bills;
CREATE POLICY "habit_bills_update_own" ON public.habit_bills FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_bills_delete_own" ON public.habit_bills;
CREATE POLICY "habit_bills_delete_own" ON public.habit_bills FOR DELETE TO authenticated USING (user_id = auth.uid());

DROP POLICY IF EXISTS "habit_badges_select_own" ON public.habit_badges;
CREATE POLICY "habit_badges_select_own" ON public.habit_badges FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS "habit_badges_insert_own" ON public.habit_badges;
CREATE POLICY "habit_badges_insert_own" ON public.habit_badges FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());