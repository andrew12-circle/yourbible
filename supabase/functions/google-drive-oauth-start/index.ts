...
Deno.serve(async (req) => {
  ...
  try {
    if (!isGoogleDriveOAuthConfigured()) {
      return new Response(JSON.stringify({ error: "Google Drive OAuth is not configured on the server." }), {
        status: 503,
    ...
    await admin.from("google_drive_oauth_states").insert({
      user_id: u.user.id,
      state,
      return_path: returnPath,
    });

    return new Response(JSON.stringify({ auth_url: buildGoogleDriveAuthUrl(state) }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
