...
import { callArtifactAnalysisModel } from "../_shared/artifactAnalysisModel.ts";
...
async function buildOverview(findings: Finding[], batches: BatchResult[]) {
  ...
  // Extraction covers every section. This bounded shortlist limits the consolidation bill,
  // not transcript coverage or the full findings list. Counts are retained for disclosure.
...
Use only valid candidate indices. Do not speak as God or a prophet.`, input,
  ...
  return {
    summary: parsed.summary.trim().slice(0, 2400),
  ...
  const work = async () => {
    ...
    try {
      if (job.stage === "extract") {
        ...
        const saved = await admin.rpc("artifact_complete_analysis_job", {
          p_job_id: job.id, p_lease_token: job.lease_token, p_result: result,
        });
        if (saved.error) throw saved.error;
      } else {
        // Paginate explicitly: default PostgREST row limits must never truncate a long source.
        ...
        const published = await admin.rpc("artifact_publish_analysis", {
          p_job_id: job.id, p_lease_token: job.lease_token, p_findings: findings, p_summary: overview,
        });
      ...
      const { error: saveError } = await admin.rpc("artifact_fail_analysis_job", {
        p_job_id: job.id, p_lease_token: job.lease_token, p_code: failure.code,
        p_message: failure.message, p_retryable: failure.retryable,
      });
      if (saveError) console.error("Could not save analysis failure", job.id, saveError.code);
    } finally {
      // Explicit run attribution; never use the shared mutable AI-usage context.
      ...
      if (run) await admin.from("ai_usage_events").insert({
        ...
        operation: `artifact_${job.stage}`, provider: "configured_analysis_provider", status: outcome,
