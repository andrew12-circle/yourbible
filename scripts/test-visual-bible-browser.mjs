/** Real back-matter page and chapter sheet. No live account or Scripture data. */
...
let browser, page;
const errors = [], externalImages = [], detailRequests = [], reports = [];
...
try {
  ...
  page.on("request", (request) => { if (request.url().includes("-detail.webp")) detailRequests.push(request.url()); });
  await page.route("**/*", async (route) => {
    const request = route.request(), url = new URL(request.url());
    if (url.origin === origin) return route.continue();
    if (request.resourceType() === "image" || url.pathname.includes("/functions/v1/bible")) externalImages.push(url.href);
  ...
  await page.locator('[data-testid="visual-results"]').waitFor({ timeout: 45000 });
  assert.equal(await page.locator("[data-visual-id]").count(), 24);
  ...
  await page.getByRole("button", { name: "View Esther before Ahasuerus", exact: true }).click();
  const dialog = page.getByRole("dialog");
  await dialog.locator("img").waitFor();
  await page.waitForFunction(() => { const image = document.querySelector('[role="dialog"] img'); return image?.complete && image.naturalWidth > 0; });
  assert.equal(detailRequests.length, 1);
  await page.screenshot({ path: join(output, "visual-detail-desktop.png") });
  await dialog.getByRole("button", { name: "Zoom in", exact: true }).click();
  assert(await dialog.getByText("150%", { exact: true }).isVisible());
  await page.keyboard.press("Escape");
  await dialog.waitFor({ state: "detached" });
  assert.equal(await page.evaluate(() => document.activeElement?.getAttribute("aria-label")), "View Esther before Ahasuerus");
  record("Actual back-matter page: bounded lazy gallery, on-demand detail, zoom and Escape focus return");
  await page.getByRole("searchbox").fill("Durer");
  assert.equal(await page.locator("[data-visual-id]").count(), 1);
  await page.getByRole("button", { name: "Clear filters", exact: true }).click();
  await page.getByRole("button", { name: "Artifacts (5)", exact: true }).click();
  assert.equal(await page.locator("[data-visual-id]").count(), 5);
  await page.getByRole("button", { name: "Clear filters", exact: true }).click();
  await page.locator("summary").filter({ hasText: "More filters" }).click();
  await page.getByRole("combobox", { name: "Filter by artist or maker" }).selectOption("Rembrandt (Rembrandt van Rijn)");
  assert.equal(await page.locator("[data-visual-id]").count(), 1);
  await page.getByRole("button", { name: "Clear filters", exact: true }).click();
  await page.locator("summary").filter({ hasText: "More filters" }).click();
  await page.evaluate(() => { document.querySelector("[data-bible-scroll]").scrollTop = 0; });
  await page.screenshot({ path: join(output, "visual-library-desktop.png") });
  record("Artist search, category filtering and expandable maker filters work in the real reader shell");
  await page.setViewportSize({ width: 390, height: 844 });
  await page.waitForTimeout(300);
  await page.evaluate(() => { document.querySelector("[data-bible-scroll]").scrollTop = 0; });
  assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), "Horizontal overflow on phone");
  assert(await page.locator("[data-visual-id] img").first().evaluate((image) => image.getBoundingClientRect().top < innerHeight * 0.75), "Phone filters pushed the first image below the fold");
  assert.equal(await page.locator("details").evaluate((details) => details.open), false);
  await page.screenshot({ path: join(output, "visual-library-mobile.png") });
  record("Phone-width collection has no horizontal overflow and artwork is visible before scrolling");
  await page.evaluate(() => window.__visualNavigate("/fixture/chapter"));
  await page.getByRole("button", { name: "Open Luke 2 visuals", exact: true }).waitFor();
  assert.equal(await page.locator("[data-visual-id]").count(), 0);
  await page.getByRole("button", { name: "Open Luke 2 visuals", exact: true }).click();
  await page.getByRole("button", { name: "View Head of Augustus", exact: true }).waitFor();
  await page.screenshot({ path: join(output, "visual-chapter-mobile.png") });
  await page.getByRole("button", { name: "View Head of Augustus", exact: true }).click();
  const detail = page.getByRole("dialog").last();
  await detail.getByText(/not evidence for a particular census/).waitFor();
  await detail.getByRole("link", { name: "Luke 2:1", exact: true }).click();
  await page.getByText("Reader navigation reached", { exact: true }).waitFor();
  record("Closed chapter sheet mounts no cards; artifact scope, caveat and reader navigation work on phone");
  ...
  for (const entry of manifest.entries) for (const file of entry.files) {
    ...
    assert(response.headers()["content-type"]?.startsWith("image/"), file.path);
    assert.equal((await response.body()).length, file.bytes, file.path);
  }
  record("Every acquired derivative is served as an image with its verified byte count");
  assert.deepEqual(externalImages, [], "Gallery made external image/Bible provider requests");
  ...
  if (page) { await page.screenshot({ path: join(output, "visual-browser-failure.png") }).catch(() => {}); console.error(JSON.stringify({ errors, reports, body: await page.locator("body").innerText().catch(() => "") }, null, 2)); }
  throw error;
