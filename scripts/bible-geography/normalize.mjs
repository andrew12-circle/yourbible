// Source format: OpenBible.info Bible-Geocoding-Data/readme.md (CC BY 4.0).
// Scores are source scores, not independently verified probabilities.
...
export function parseOsis(value) {
  ...
  const b = OSIS.indexOf(match[1]) + 1;
...
function normalizeVerse(verse, placeId) {
  ...
  const c = Number(key.slice(2, 5));
  const v = Number(key.slice(5, 8));
  ...
  const a = {};
  for (const [translation, reference] of Object.entries(verse.alternate_verses ?? {})) {
    if (TRANSLATIONS.includes(translation)) a[translation] = parseOsis(reference);
  }
  return { b, c, v, t, ...(Object.keys(a).length ? { a } : {}) };
}

export function normalizePlace(raw) {
  if (!/^a[0-9a-f]{6}$/.test(raw.id) || !raw.friendly_id || !Array.isArray(raw.verses)) {
    throw new Error(`Invalid ancient place: ${raw.id}`);
  }
  const candidates = [];
  const unresolved = [];
  const seen = new Set();
  for (const [i, identification] of (raw.identifications ?? []).entries()) {
    ...
    for (const [j, resolution] of (identification.resolutions ?? []).entries()) {
      ...
      const association = raw.modern_associations?.[modernId];
      ...
      if (score !== null && (!Number.isFinite(score) || score < 0 || score > 1000)) throw new Error(`Invalid source score for ${raw.id}`);
      candidates.push({
        id: `${raw.id}-${i}-${j}`,
        modernId: modernId || '',
        name: plain(association?.name || description),
        description,
        ...coordinates,
        coordinateKind,
        score,
        radiusMeters: resolution.geometry_radius_meters ?? identification.geometry_radius_meters ?? null,
        geometryId: resolution.geometry_id ?? identification.geometry_id ?? null,
      });
    }
  }
  candidates.sort((a, b) => (b.score ?? -1) - (a.score ?? -1) || a.name.localeCompare(b.name) || a.id.localeCompare(b.id));
  const uniqueUnresolved = [...new Map(unresolved.map((item) => [`${item.kind}|${item.description}`, item])).values()];
  return {
    id: raw.id,
    name: plain(raw.friendly_id),
    aliases: Object.keys(raw.translation_name_counts ?? {}).map(plain).sort(),
    type: plain(raw.type || 'place'),
    sourceUrl: `https://www.openbible.info/geo/ancient/${raw.id}/${encodeURIComponent(raw.url_slug || raw.friendly_id.toLowerCase().replaceAll(' ', '-'))}`,
    notes: plain(raw.comment),
    geojsonFile: raw.geojson_file || null,
    candidates,
    unresolved: uniqueUnresolved,
    references: raw.verses.map((verse) => normalizeVerse(verse, raw.id)),
  };
}
