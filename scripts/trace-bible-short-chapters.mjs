...
source=source.replace('resolve:{alias:[', "resolve:{alias:[{find:'@/hooks/useReaderContinuation',replacement:join(scratch,'continuation-trace.ts')},{find:'@/lib/bible/readerContinuation',replacement:join(scratch,'demand-trace.ts')},");
...
source=source.replace(diagnostic, `${diagnostic}continuation:await page.evaluate(()=>({history:window.__continuationHistory,demand:window.__continuationDemand,path:window.__path,rootBusy:document.querySelector('[data-bible-reader]')?.getAttribute('aria-busy'),busy:[...document.querySelectorAll('[aria-busy="true"]')].map(n=>({tag:n.tagName,classes:n.className,outer:n.outerHTML.slice(0,300)}))})),`);
