...
replaceSection('  const scenarios = [', '  for (scenario of scenarios)', `  const scenarios=[
    {name:'acts-single',columns:'single',study:'inline',scale:1,width:1491,height:936},
    {name:'acts-double',columns:'double',study:'inline',scale:1,width:1491,height:936}
  ];
`);
replaceSection('    if (scenario.oversized)', '  assert.deepEqual(browserErrors,[]);', `    for(let step=0;step<9;step++) {
      const current=await inspect();
      const extra=await page.evaluate(()=>({history:window.__readerPositionHistory,
        html:[...document.querySelectorAll('[data-reader-page-side] article')].map(n=>n.outerHTML)}));
      steps.push({step,...current,position:extra.history.at(-1)});
      console.log(JSON.stringify({scenario:scenario.name,step,...current,position:extra.history.at(-1)}));
      if(step<5) await page.screenshot({path:join(output,scenario.name+'-'+step+'.png')});
