/** Fail a release whose generated visual files were not actually copied to dist. */
...
export async function verifyVisualBibleDist({ root = ROOT, outDir = "dist" } = {}) {
  ...
  for (const asset of assets) {
    ...
    const expected = asset.imageUrl.startsWith("/") ? [asset.imageUrl]
      : ["thumb", "detail"].map((size) => `/visual-bible/v1/${asset.id}-${asset.revision}-${size}.webp`);
